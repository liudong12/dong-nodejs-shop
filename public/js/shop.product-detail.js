(function($) {  
    // var dongliu = 'b';
    // alert(dongliu);
    //var rootUrl = "http://momoteks.com/projects/dong-shop";
    ...
    $(".imgbox").mousemove(function(event) {
        ...
        var sx = event.pageX, //鼠标在页面上的位置
            ...
            cHeight = sy - oy,
            simgwidth = $("#smallimg").width(), //小图片的宽高  //本来是 #smallimg
            simgheight = $("#smallimg").height(),               //本来是 #smallimg
            //大图片的宽高
            bimgwidth = $("#bigimg").width(),           //本来是 #bigimg
            bimgheight = $("#bigimg").height(),         //本来是 #bigimg
            //大图片显示区域的大小的一半
    ...
    //当鼠标点击缩略图时，在上方显示对应的图片 //切换不同角度的商品图片
    ...
    //得到初始的颜色的名字//.default=>.color
    var dataColor = $(".product-color-ul").find(".selected").find(".color").data("color");
    ...
    $(".product-color-li").click(function() {
        ...
        $.ajax({
            url:    rootUrl + url + '/' + productId, // 跳转到 action //Type: String //A string containing the URL to which the request is sent.   
            // data: {
    ...
    $(".product-size-li").click(function() {
        $( '.add_cart_errors' ).hide();  //隐藏错误提示信息 
        $(this).addClass("size-selected").siblings().removeClass("size-selected");//被点击的加边框，其他的去除边框
        var dataSize = $(this).find(".size").data("size").toUpperCase();//得到被点击的那个尺寸的名字
        // alert(dataSize);
    ...
    $(".product-color-ul").find(".selected").find("input").attr('checked', true);
     ...
     var options = {  
        //url : '/add-to-cart',//默认是用form里面的请求地址  //action="{{route('product.addToCart')}}"
         ...
         success : function (responseText) {  //alert("success");
            //如果是修改的话，就刷新页面
            if(responseText.isUpdate){
                location.reload();
            }
            else{  
                //alert(responseText.itemsInCart);
                
                // $(".badge").text('(' + responseText.itemsInCart + ')');//更新购物车里商品的数量  //加括号
                // $(".badge").text(responseText.itemsInCart);//更新购物车里商品的数量 //不加括号

                // $('.reload').load(document.URL +  ' .mobile-cart');
                // $(".mobile-cart-trigger").trigger("click");
            
                //alert(responseText.itemsInCart);
                if($(".desktop-cart-wrapper").is(":visible")){  //如果是电脑端
                    $(".badge").text('(' + responseText.itemsInCart + ')');//更新购物车里商品的数量  //加括号
                    $("#reload-desktop-cart").load(rootUrl +  ' .desktop-cart', function(responseTxt, statusTxt, xhr){
                        if(statusTxt == "success"){
                            //alert("External content loaded successfully!");
                            $(".desktop-cart-trigger").trigger("mouseover");
                            setTimeout(function(){
                                
                                var cartIsVisible = ( !$('.desktop-cart').is(":visible") ) ? true : false;
                                
                                if (!cartIsVisible){//如果还没有关闭
                                    $(".desktop-cart-trigger").trigger("mouseout"); 
                                }else{
                ...
                else if($(".mobile-cart-wrapper").is(":visible")){ //如果是手机端 
                    $(".badge").text(responseText.itemsInCart);//更新购物车里商品的数量 //不加括号
                    ...
                    //  
                    //  纠正：
                    //  本地： '/'   表示 http://dong-shop.com:8000
                    //  godaddy:  '/'   表示 http://momoteks.com
                    //  http://momoteks.com/projects/dong-shop
                    //  rootUrl === http://momoteks.com/projects/dong-shop
                    $("#reload").load(rootUrl +  ' .mobile-cart', function(responseTxt, statusTxt, xhr){
   ...
   // });
    ...
    //如果要前端验证的话，用这一段，如果不用的话，用上面那段
    ///////////////////////////////////////////////////////////////////
    $("#add-to-cart").validate({ //备注：如果要用validate来验证的话，form里面就不能写action
        debug: true,
        rules: {
                // gender: "required",
        ...
        submitHandler: function(form) {  
        //alert('aaa');    
                $("#add-to-cart").ajaxSubmit(options);  
