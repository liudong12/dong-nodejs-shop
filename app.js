//this file is the first place each request will come to
...
var expressHbs = require('express-handlebars'); //导入一个更强大的模板引擎
...
//注意：你不需要去创建叫 shopping 的数据库，这样写好以后，会自动创建的 (you don't have to create the database, it will created automatically for you if it doesn't exist yet)
...
//本来的有点问题： 这里应该是 注册模板引擎，参数一是引擎的名字 .hbs
app.set('view engine', '.hbs');//使用上面这句话生成的引擎对象 handlebars //应该是 .hbs
...
app.use(session({//here, we store the session on mongodb database. 保存在数据库中的到期的session将被自动的删除
  ...
  saveUninitialized: false, //the default value is true. if this is set to true, the session will be stored on the server even through it is not been initialized, nothing has been added there.
